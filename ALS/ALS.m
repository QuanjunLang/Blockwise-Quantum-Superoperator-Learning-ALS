...
for iter = 1:maxIter
    ...
    switch Acceleration
        case 'None'
            U1 = update_U(A, b, V0, M, N, r);
            V1 = update_V(A, b, U1, M, N, r);
        case 'Polyak'
            U1_temp = update_U(A, b, V0, M, N, r);
            V1_temp = update_V(A, b, U1_temp, M, N, r);
        ...
        case 'Nesterov'
            ...
            if iter > 3
                ...
                U1 = update_U(A, b, V_Nesterov, M, N, r);
    ...
    X1 = U1 * V1'; % Current estimate of M
    ...
    if iter > 5 && (rel_error < rel_err_tol || loss(iter) < loss_tol ) && (loss(iter) < 1e-2)
...
function U = update_U(A, b, V0, M, N, r)
% Step 1: Update U with fixed V
regmat_U = zeros(M, N*r);
for m = 1:M
    regmat_U(m, :) = vec(A(:, :, m) * V0)'; % Matrix product A_i * V
...
function V = update_V(A, b, U0, M, N, r)
% Step 2: Update V with fixed U
