...
%%
temp = load('4_1_Lindbladian_typical_data.mat', 'all_form');
...
frob_err = strcat(num2str(mean_small(:, 1), '%.2e'), ' $\pm$ ', num2str(std_small(:, 1), '%.2e'));
diam_err = strcat(num2str(mean_small(:, 2), '%.2e'), ' $\pm$ ', num2str(std_small(:, 2), '%.2e'));
...
%%
temp = load('4_1_Lindbladian_typical_data_Large.mat', 'all_form');
all_form = temp.all_form;

mean_Large = mean(all_form, 3);
std_Large  = std(all_form, 0, 3);

frob_err = strcat(num2str(mean_Large(:, 1), '%.2e'), ' $\pm$ ', num2str(std_Large(:, 1), '%.2e'));
...
%%
temp = load('4_1_Lindbladian_typical_data_Ex_Large_all_form.mat', 'all_form');
all_form = temp.all_form;
...
frob_err = strcat(num2str(mean_ex_Large(:, 1), '%.2e'), ' $\pm$ ', num2str(std_ex_Large(:, 1), '%.2e'));
...
writetable([T1;T2;T3], '4_1_Lindbladian.csv')
